...
def createPosMap(ann, posRefStart, posRefEnd, posSimStart, posSimEnd):
    ...
    posRefStart.setdefault(ann.ref_chr, []).append(ann.ref_end)
    posRefStart.setdefault(ann.ref_chr, []).append(ann.sim_start)
    posRefStart.setdefault(ann.ref_chr, []).append(ann.sim_end)
    
def simPos2Ref(chrom, posInSim, posRefStart, posRefEnd, posSimStart, posSimEnd): 
    posSimStartVec = posSimStart[chrom]
    i = bisect.bisect_right(posSimStartVec, posInSim)
    l_posRef = len(posSimStartVec[chrom])
    if i >= l_posRef:
        print >>sys.stderr, 'len of posRef '+str(len(posRef[chrom])) 
        print >>sys.stderr, i
        #sys.exit(1)
        return 0  
    eventPosInRef = posRefStartVec[chrom][i]
    eventPosInSim = posSimStartVec[chrom][i]
    if i < l_posRef-1:
        if eventPosInSim > posSim and posSimStartVec[chrom][i+1] >posSim and\
            eventPosInSim - posSim >= posSimStartVec[chrom][i+1] -posSim:
...
if __name__ == '__main__':
    ...
    for line in fp_ann:
        a = read_ann.ann(line)
        createPosMap(a, posRefStart, posSimEnd, posSimStart, posSimEnd)
        #print posRef, posSim
    #convert read1
    tot_num = 0
